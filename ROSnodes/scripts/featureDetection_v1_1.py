...
def img_rec():
    rospy.init_node('image_reciever',anonymous=False)
    rospy.Subscriber("raw_img",Image,callback,queue_size=1)
    rospy.spin()

def callback(ros_data):
    bridge=CvBridge()
    #print 'recieved data', ros_data
    recievedImage = bridge.imgmsg_to_cv2(ros_data,"bgr8")
    save(recievedImage)
